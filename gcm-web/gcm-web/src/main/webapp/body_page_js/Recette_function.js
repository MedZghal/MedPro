...
//                    this.$el.css({background:"repeating-linear-gradient(45deg,#606dbc,#606dbc 10px,#465298 10px,#465298 20px"});
    ...
    var CaptionFooter = Backgrid.Footer.extend({
            ...
            render: function () {
              this.el.innerHTML = '<tr colspan="7"><td colspan="4" style="color:#23527c;">Total</td><td>'+(recettetoday-depense).toFixed(3)+' DT</td><td>'+depense.toFixed(3)+' DT</td></td><td>'+((recettetoday-depense)-depense).toFixed(3)+' DT</td></tr>';
              return this;
            }
