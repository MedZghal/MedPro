...
var QuickSidebar = function () {
    ...
    var handleQuickSidebarChat = function () {
        ...
        var handleChatMessagePost = function (e) {
            ...
            var Code_med=JSON.parse(localStorage.getItem("paramater")).codeMedTrit;
            var envoieUser = GetListUtilisateurByMedecin(Code_med.codeMedTrit);
            var Secretaire =JSON.parse(localStorage.getItem("Secretaire"));
            if(Object.keys(Secretaire).length>0){
                $.ajax({
                        url: "/GEMP/ReciveMsg?msg="+text+"&user="+Code_med.username,
                        type: "POST"
                });
                $.ajax({
                    url: "/GEMP/ReciveMsg?msg="+text+"&user="+Secretaire.username,
                    type: "POST"
                });
            }else{
                $.each(envoieUser , function (i){
                    $.ajax({
                        url: "/GEMP/ReciveMsg?msg="+text+"&user="+envoieUser[i].username,
                        type: "POST"
                    });
                });
            }
...
//                if(Object.keys(Secretaire).length>0)
//                    tpl += '<img class="avatar" alt="" src="../img/avatars/nurse.png"/>';
//                else
//                    tpl += '<img class="avatar" alt="" src="../img/avatars/doctor-.png"/>';
//                tpl += '<div class="message">';
//                tpl += '<span class="arrow"></span>';
//                tpl += '<a href="#" class="name">'+name+'</a>&nbsp;';
//                tpl += '<span class="datetime">' + time + '</span>';
//                tpl += '<span style=" word-wrap: break-word; " class="body">';
//                tpl += message;
//                tpl += '</span>';
//                tpl += '</div>';
//                tpl += '</div>';
...
//                return tpl;
//            };
//            if(envoieUser !== Utilisateur.user){
//            // handle post
//            var time = new Date();
//            var message = preparePost('out', (time.getHours() + 'h' + time.getMinutes()),Utilisateur.user, text);
//            message = $(message);
